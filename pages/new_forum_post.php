...
if (isset($_POST['titlePHP'])) {
    ...
    try {
        if (isset($_POST['organisationIdPHP'])) {
            ...
            $sql = "INSERT INTO threads (title, content, user_id, organisation_id) VALUES (:title, :content, :user_id, :organisation);";

            // query database and insert the new announcement into the announcements table
            $stmt = $connectionPDO->prepare($sql);

            //check to see if the insert was successful
            if ($stmt->execute(['title' => $title, 'content' => $content, 'user_id' => $user_id, 'organisation' => $organisation_id])) {
                try {
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#new-post-submit").on('click', function () {
                ...
                let organisationID = '<?php echo $_GET['orgid']; ?>' || undefined;
                ...
                if (title == "" || content == "") {
                    ...
                    $.ajax({
                        ...
                        data: {
                            titlePHP: title,
                            contentPHP: content,
                            organisationIdPHP: organisationID
                        },
